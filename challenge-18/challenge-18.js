(function () {
    ...
    */
    ...
    let matchRegex = 'Os meses de janeiro, junho e julho começam com a letra j.'.match(/(ju\w+)/g);
    ...
    */
    ...
    let matchTags = "<div><section><blockquote>Texto <img /></blockquote></section></div>".match(/(<\w+>)/g);
    ...
    */
    ...
    let regexTagHTML = '<div><ul><li></li><li></li><li><span></span></li></ul></div>'.match(/(<\w+><\/\w+>)/g);

    console.log(regexTagHTML);
    ...
    */
    ...
    let regexTagsText = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
        .replace(
            /(<(\w+)>)([\w éíá]+)(<\/\w+>)/g,
            '$1O texto dentro da tag "$2" é "$3"$4\n');
    //Fazendo como o daciuk
    let regexDaciuk = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
        .replace(
            /<(\w+)>([^<]+)<\/\w+>/g,
            '<$1>O texto dentro da tag "$1" é "$2"</$1>\n');

    console.log(regexTagsText);
    console.log(regexDaciuk);
